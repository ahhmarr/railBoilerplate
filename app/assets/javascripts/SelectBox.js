...
(function($){
	...
	window.SelectBox = function(options){
		...
		cfg = $.extend(true, {
			...
			zIndex: 100,
			changeCallback: function(val,obj) { },
		...
		function _updateValue(val) {
			if($selectedValue.html() != val) {
				...
				cfg.changeCallback(cfg.selectbox.val(),cfg.selectbox);
			}
		}
		...
		function _renderOptions() {
			...
			$options.each(function(i, itm) {
				...
				style=($(itm).attr("style"))?" style='"+$(itm).attr("style")+"' ":'';
				if(itm.selected) {
					_selectedValue = iconMarkup + _truncate($(itm).html());
					optionHTML.push('<dd class="itm-'+i+' selected " '+style+'>' + iconMarkup + itm.innerHTML + '</dd>');
				} else {
					optionHTML.push('<dd class="itm-'+i+'" '+style+'>' + iconMarkup + itm.innerHTML + '</dd>');	
				}
			});
